...
define	TASKDICT	"|display|imcentroid|selectpars|tpltsol|tvmark_"
...
define	LOGCMD	"finderlog %s %s append+ >& dev$null"

define	FITCMD	"tpltsol %s %s %s imupdate=%s tabupdate+ refitcat=%s dssheader- ra_ref=%s dec_ref=%s eq_ref=%g"

define	CMDDICT	"|autodisplay|boxsize|badcolor|goodcolor|eparam|marker|omarker|replace|rotate|scale|selectpars|shift|show|subsample|update"

define	KEYHELP		"finder$lib/scr/tpeak.key"
...
bool	update, autodisplay, autocenter, interactive, redisplay, every_source
real	pangle, eq_ref
...
begin
	...
	subsample = clgeti ("disppars.subsample")
	frame = clgeti ("disppars.frame")

	call clgstr ("disppars.marker", mark, SZ_NAME)
	call clgstr ("disppars.omarker", ovmark, SZ_NAME)

	call clgstr ("disppars.goodcolor", gcolor, SZ_NAME)
	call clgstr ("disppars.badcolor", bcolor, SZ_NAME)
	call clgstr ("disppars.objcolor", ocolor, SZ_NAME)
	...
	if (Memc[objects] != EOS && strmatch (Memc[objects], "^#$") == 0) {
		...
		while (getline (ob, Memc[buf]) != EOF)
			...
			if (nscan () == 3) {
			    ...
			    if (! IS_INDEFR (cx) && ! IS_INDEFR (cy))
				call newobject (tp, cp, nstars, cx,cy, nstars+1)
			}
		    }
	    }
	...
	while (clgcur ("imcur", wx, wy, junk, key, Memc[cmd], SZ_LINE) != EOF) {
	    ...
	    call tbrgti (tp, cp[OBJECT], objflag, bunk, 1, index)
	    isobj = (objflag == 1)
	...
	call printf ("\007\nThe nearest source is a program object and\n")
	call printf ("  the 'c' keystroke only applies to catalog sources.\n")
	call printf ("  Please reposition the cursor and try again.\n")
		    call flush (STDOUT)
	    ...
	    # tpltsol writes to the table, so have to close it first
	    case 'e':
		call tbtclo (tp)
		call tpltsol (Memc[image], Memc[table], Memc[database], false,
		    true, Memc[ra_ref], Memc[dec_ref], eq_ref)
#call printf ("outside tpltsol\n")
#call flush (STDERR)
		nstars = opentab (Memc[table], tp, cp)
		if (autodisplay)
		    redisplay = true
		...
		call tpltsol (Memc[image], Memc[table], Memc[database], update,
		    false, Memc[ra_ref], Memc[dec_ref], eq_ref)
		nstars = opentab (Memc[table], tp, cp)
		if (autodisplay)
		    redisplay = true
	...
	call printf ("\007\nThe nearest source is a program object and\n")
	call printf ("  the 'j' keystroke only applies to catalog sources.\n")
	call printf ("  Please reposition the cursor and try again.\n")
		    call flush (STDOUT)
		...
		if (isobj) {
	call printf ("\007\nThe nearest source is a program object and\n")
	call printf ("  the 'k' keystroke only applies to catalog sources.\n")
	call printf ("  Please reposition the cursor and try again.\n")
		    call flush (STDOUT)
		    next
		}
		...
		if (isobj) {
	call printf ("\007\nThe nearest source is a program object and\n")
	call printf ("  the 'l' keystroke only applies to catalog sources.\n")
	call printf ("  Please reposition the cursor and try again.\n")
		    call flush (STDOUT)
		    next
		}
	    ...
	    case 'p':
                if (! isobj) {
        call printf ("\007\nThe nearest source is a catalog object and\n")
        call printf ("  the 'p' keystroke only applies to program objects.\n")
        call printf ("  Please reposition the cursor and try again.\n")
                    call flush (STDOUT)
                    next
                }
...
#		    call overlay_one (frame, cx, cy, mark, bcolor)
		    ...
		    call eprintf ("\007Centering failed at (%7.2f,%7.2f).\n")
			call pargr (cx)
			call pargr (cy)
		    call flush (STDERR)
		}
...
#	call mktemp ("tmp$JUNK", Memc[tmp], SZ_FNAME)
	call mktemp ("tmp$JJJ", Memc[tmp], SZ_FNAME)
...
#call flush (STDOUT)
		...
		;
	    else if (valid == ISOBJ && Memi[c+i-1] == YES && Memi[o+i-1] == YES)
		;
	    else
		next
...
begin
	...
	call tbrgti (tp, cp[OBJECT], objflag, junk, 1, index)
	if (objflag == 1) {
	    call tbrdel (tp, index, index)
	    n = n - 1
	} else {
...
bool	disjunction, firsttime, is_string

pointer	clopset()
bool	clgpsetb(), is_column()
int	strmatch(), stridx()
...
#	    disjunction = clgpsetb (pp, "disjunction")
		...
		if (is_column (tp, Memc[buf], is_string)) {
			...
			if (Memc[buf] != EOS && strmatch(Memc[buf],"^#$")==0) {
				...
				call printf ("\nQuotes not allowed (%s) - ")
				    call pargstr (Memc[buf])
				call printf ("item skipped\n")
				call flush (STDOUT)
			    }
			}
		    }
		    ...
		    call printf ("\nTable column %s not found - item skipped\n")
			call pargstr (Memc[buf])
		    call flush (STDOUT)
		}
	    }
	    ...
	    if (firsttime) {
		call printf ("\nNo expression given, selecting all entries\n")
		    call pargstr (Memc[buf])
		call flush (STDOUT)
	...
	call tbtflu (tp)
        ...
        call printf ("\nNew subset selected using selectpars expression:\n")
	call printf ("  %s\n")
...
# IS_COLUMN -- does the named column exist in the table?
# A bool argument is set to true for string type columns.

bool procedure is_column (tp, colname, is_string)

pointer	tp			#I table pointer
char	colname			#I column name
bool	is_string		#O Is the column a string?

pointer	cp

int	tbcigi()

errchk	tbcfnd, tbcigi
...
begin
	...
	if (cp == NULL) {
	    ...
	    is_string = (tbcigi (cp, TBL_COL_DATATYPE) < 0)
	    return (true)
	}
end
